...
class AdminSegundaViaController extends Controller {
	...
	{
		view()->share('page_title','2º Via de Boletos');
		view()->share('page_description','Emissão de 2º via de Boletos e Intimações.');
	}
	...
	public function index()
	{
		...
		$registros = $registros->orderBy('arquivo_remessa_boleto_dets.data_emissao_titulo','desc',DB::raw('date_format(STR_TO_DATE(vencimento_boleto,"%d%m%Y"),"%d/%m/%Y")'),'desc')
                           	   ->paginate(10);
	...
	public function store()
	{
		...
		fclose($file);
		return redirect('admin/segundavia')->with('sucesso',true);
	}
	...
	{	
		$registro = ArquivoBoletoDet::find($id);
		$registro->impresso = 1;
		$registro->save();
	...
	public function segundaviapublica(Request $request)
	{	
		$id = Input::get('protocolo');

		$registro = ArquivoBoletoDet::where('protocolo', '=', $id)->first();
		if($registro){
			$registro->impresso = 1;
			$registro->save();
			...
			return view('ieptbma::segundavia.visualizar')
			->with('boleto',$boleto);	
		} else {
			return Redirect::back()->withErrors(['message1', 'My message']);
		}
	}
