...
<div class="col-xs-12">
    ...
    <div class="box box-primary">
        <div class="box box-header">
            <div class="pull-left">
                <a href="{{ url('downloadExcelCredenIntimacao/xlsx?'.http_build_query(request()->input(),'','&'))}}" class="btn btn-success pull-right" style="margin-left: 10px">Gerar Arquivo em Excel</a>
            </div>
            <form action="{{ url('admin/credenciamentoboleto') }}" method="GET" id="formFilter">
                ...
                <div class="pull-right"  style="margin-right: 40px">
                    <p><strong>Status do Credenciamento</strong></p>
                    <label class="radio-inline">
                        ...
                        <?php foreach ($usuarioCredenciamento as $key => $value): ?>
                            <tr>
                                ...
                                @if (Auth::user()->papel_id <= 2)
                                    <td>
                                        @if($value->verified == 0)
                                            <span class="badge badge-danger" style="background-color: #ffc107">Não verificado</span>
                                        @else
                                            <span class="badge badge-success" style="background-color: #407020">Verificado</span>
                                        @endif
                                    </td>
                                @endif
                                <td>
                                    <a class="btn btn-primary" target="_blank" href="{{ url('admin/segundavia') }}?pesquisar={{ $value->cpf}}&campo=credenpj&razao={{ $value->name }}&cnpj={{ $value->cpf }}" title="Visualizar">Visualizar intimações</a>
                                </td>
                                <td>
                                    @if (Auth::user()->papel_id <= 2 && $value->verified == 0)
                                        <a class="btn" href="{{url('admin/credenciamentoboleto/verificaemail/'.$value->id)}}" data-toggle="tooltip" data-placement="bottom" title="Enviar E-mail de verificação"><i style="font-size: 15px" class="fa fa-paper-plane"></i></a>
                                    @endif
                                    <a class="btn" href="{{url('admin/credenciamentoboleto/'.$value->id)}}" title="Visualizar"><i style="font-size: 15px" class="fa fa-search"></i></a>
                                    <a class="btn" href="{{url('admin/credenciamentoboleto/'.$value->id).'/0/edit '}}" title="Editar"><i style="font-size: 15px" class="fa fa-edit"></i></a>
                                    @if(Auth::user()->papel_id <= 2)
                                        <a class="btn" href="{{url('admin/credenciamentoboleto/'.$value->id).'/1/edit'}}" title="Credenciar" data-toggle="tooltip" data-placement="bottom" title="Credenciar"><i style="font-size: 15px; color: #407020" class="fa fa-thumbs-o-up"></i></a>
                                        ...
                                        <div class="modal fade" id="model_{{ $value->id }}" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true" role="dialog">
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{url('admin/credenciamentoboleto/'.$value->id).'/2/edit'}}" style="display: inline">
                                        ...
                                        <a class="btn" href="{{url('admin/credenciamentoboleto/'.$value->id)}}" data-method="delete" 
